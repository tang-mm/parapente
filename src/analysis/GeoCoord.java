...
public class GeoCoord {
	...
	public static double convertLatitudeFromString (String latStr) throws Exception {
		...
		int lats = Integer.parseInt(latStr.substring(4, 7));
		
		double lat = Math.abs(latd) + latm/60.0 + lats/3600.0;
		if (lat < 0) return -lat;
		else return lat;
	}
	...
	public static double convertLongitudeFromString (String lngStr) throws Exception {
		...
		int lngs = Integer.parseInt(lngStr.substring(5, 8));
		
		double lng = Math.abs(lngd) + lngm/60.0 + lngs/3600.0;
		if (lng < 0) return -lng;
		else return lng;
	}
}
